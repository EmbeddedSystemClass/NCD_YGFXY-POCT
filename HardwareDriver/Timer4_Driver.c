...
void TIM4_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM4,TIM_IT_Update)==SET) //????
	{
		...
		if(getSystemRunTimeData()->motorData.motorDir == Forward)
			setSystemMotorLocation(getSystemRunTimeData()->motorData.location + 1);
		else
			setSystemMotorLocation(getSystemRunTimeData()->motorData.location - 1);
		
		if(BackLimited)
			setSystemMotorLocation(0);
		
		if(PreLimited)
		{
			setSystemMotorLocation(60000);
		}
